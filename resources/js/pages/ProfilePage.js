...
class ProfilePage extends Component {
    constructor() {
        ...
        this.state = {
            posts: [],
            isLoading: 'text-center loadingImage',
            isError: 'wentWrongImage',
            isNull: 'd-none',
            messageBox: "d-none"
        }
    }
    componentDidMount() {
        ...
        Axios.get('/getPosts').then(response => {
            if(response.status == 200) {
                this.setState({posts: response.data, isLoading:'text-center loadingImage'})
            }
            else if(response.data == null) {
                this.setState({isLoading:'d-none', isNull: 'text-center'})
            } else {
                this.setState({isLoading:'d-none', isError: 'contentRow text-center errorRow'})
            }
        }).catch(error => {
            this.setState({isLoading:'d-none', isError: 'text-center wentWrongImage'})
        })
    }
                ...
                <MainLayout>
                    <Container>
                        <div className="profile">
                            ...
                            {/*                <Button className="btn btn-primary postBtn" onClick={this.postFunction}>Add Post</Button>*/}
                            {/*            </Col>*/}
                            {/*        </Row>*/}
                            {/*    </Col>*/}
                            {/*</Row>*/}
                            ...
                            <div>
                                <Row className={this.state.messageBox}>
                                    <img src={loadingImage} className={this.state.isLoading}/>
                                </Row>
                                <Row className={this.state.messageBox}>
                                    <img src={errorImage} className={this.state.isError}/>
